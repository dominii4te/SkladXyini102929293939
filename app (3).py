...
def add_audio_to_video(original_video_path, video_no_audio_path, output_path):
    ...
    try:
        ...
        if result.returncode != 0:
            return False, result.stderr[-500:]  # последние 500 символов ошибки
...
def frames_to_video_fast(frames_dir, output_path, fps):
    ...
    if result.returncode != 0 or not os.path.exists(output_path):
        raise RuntimeError(f"ffmpeg frame assembly failed: {result.stderr[-500:]}")
...
def _process_one_video(args):
    ...
    raw_name = os.path.basename(video_path) if isinstance(video_path, str) else f"vid_{i}.mp4"
    # Убираем потенциально проблемные символы для ffmpeg/filesystem, оставляем расширение
    name_stem, name_ext = os.path.splitext(raw_name)
    safe_stem = "".join(c if c.isalnum() or c in "-_" else "_" for c in name_stem)[:80]
    vid_name = f"{safe_stem}{name_ext or '.mp4'}"
    ...
    if add_audio:
        ok, err = add_audio_to_video(local_copy, out_no_audio, out_final)
        if not ok:
            print(f"[WARNING] Audio mux failed for video {i} ({vid_name}): {err}")
    ...
    if not os.path.exists(result_path):
        raise RuntimeError(f"Video {i} ({vid_name}) processing produced no output file at {result_path}")
